...
class TransformerLM(torch.nn.Module):
    ...
    def sampling_ids(
        ...
        if ignore_eos is True:
            weighted_scores[self.eos_token] = -float('inf')
...
class Qwen2Encoder(torch.nn.Module):
    ...
    def forward_one_step(self, xs, masks, cache=None):
        outs = self.model(
            inputs_embeds=xs,
            # Every sequence in autoregressive inference is unpadded. Let
            # Transformers build the causal mask from the KV-cache length;
            # a one-token mask is ambiguous once the cache is non-empty.
            attention_mask=None,
            output_hidden_states=True,
            return_dict=True,
            use_cache=True,
...
class Qwen2LM(TransformerLM):
    ...
    @torch.inference_mode()
    def inference_wrapper(self, lm_input, sampling, min_len, max_len, uuid, generation_state=None):
        if generation_state is not None:
            generation_state.update(stop_token_id=None, stopped_by_eos=False, hit_max_length=False)
            ...
            while True:
                ...
                if self.vllm_output_queue[uuid].empty() is False:
                    ...
                    if top_ids in self.stop_token_ids:
                        if generation_state is not None:
                            generation_state.update(stop_token_id=int(top_ids), stopped_by_eos=top_ids == self.eos_token)
                        break
                    # in stream mode, yield token one by one
                    yield top_ids
                    out_tokens.append(top_ids)
                    if len(out_tokens) == max_len:
                        if generation_state is not None:
                            generation_state['hit_max_length'] = True
                        break
                time.sleep(0.001)
            with self.lock:
