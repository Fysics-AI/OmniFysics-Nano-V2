...
def get_dynamic_batching_sample_len(item: Dict[str, Any]):
    sample_len = item["attention_mask"].sum()
    if "target_speech_token_len" in item:
        sample_len = sample_len + item["target_speech_token_len"].sum()
    return sample_len
...
class DynBszBuffer:
    ...
    def append(self, item: Dict[str, Any]):
        ...
        self._buffer_sample_lens.append(get_dynamic_batching_sample_len(item))
                ...
                if isinstance(e, StopIteration):
                    if self.step < self._length:
                        ...
                        try:
                            ...
                            if not self._drop_last and not self.batching_strategy.empty():
