...
from veomni.utils.single_process import get_runtime_state, identity_layout
...
class Qwen3_5DynamicCache:
    ...
    """
    # 一个动态缓存，可以同时处理注意力缓存（维度为 seq_len）和线性注意力缓存
    # （形状固定，与 seq_len 无关）。

    # 该缓存包含两组张量列表：`key_cache` 和 `value_cache` 用于注意力缓存，`conv_states`
    # 和 `ssm_states` 用于门控 DeltaNet 缓存。每个列表包含 `num_layers` 个张量。每个张量的预期形状如下：
    ...
    # 对于线性注意力层，`key_cache` 和 `value_cache` 的形状为 `(batch_size, 0)`（空张量）。张量），
    # 其中 `conv_states` 表示卷积状态，形状为 `(batch_size, d_inner, d_conv)`，
    # 而 `recurrent_states` 表示循环状态，形状为 `(batch_size, d_inner, d_state)`。
...
class Qwen3_5GatedDeltaNet(nn.Module):
    ...
    def forward(
        ...
        ulysses_enabled = get_runtime_state().ulysses_enabled
        if ulysses_enabled:
            ulysses_group = get_runtime_state().ulysses_group
            ulysses_size = get_runtime_state().sp_size
            ulysses_rank = get_runtime_state().sp_rank
            ...
            q_proj = identity_layout(q_proj, seq_dim=1, head_dim=2, group=ulysses_group)
            k_proj = identity_layout(k_proj, seq_dim=1, head_dim=2, group=ulysses_group)
            v_proj = identity_layout(v_proj, seq_dim=1, head_dim=2, group=ulysses_group)

            b = b.reshape(batch_size, seq_len, self.num_v_heads)
            a = a.reshape(batch_size, seq_len, self.num_v_heads)
            b = identity_layout(b, seq_dim=1, head_dim=2, group=ulysses_group)
            a = identity_layout(a, seq_dim=1, head_dim=2, group=ulysses_group)
        ...
        if ulysses_enabled:
            core_attn_out = identity_layout(
                core_attn_out, head_dim=2, seq_dim=1, group=get_runtime_state().ulysses_group
            )
