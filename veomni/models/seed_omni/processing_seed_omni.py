...
class SeedOmniProcessor(ProcessorMixin):
    ...
    def __init__(self, tokenizer=None, chat_template=None, **kwargs):
        ...
        for prefix in self.processor_prefixes:
            processor_attr = f"{prefix}_processor"
            setattr(self, processor_attr, kwargs.pop(processor_attr, None))

        if "audio_tokenizer" in kwargs:
            self.audio_tokenizer = kwargs.pop("audio_tokenizer")
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, **kwargs):
        ...
        for prefix in cls.processor_prefixes:
            if exists(pretrained_model_name_or_path + f"/{prefix}_processor"):
                sub_processor = build_processor(os.path.join(pretrained_model_name_or_path, f"{prefix}_processor"))
