...
def _get_registered_processor_class_name(processor_path: str) -> str | None:
    """Read processor_config.json first so custom processor dirs can be loaded directly.

    AutoProcessor may eagerly resolve a saved custom processor directory to a base HF
    processor class. That breaks replacement for Seed-Omni custom processors whose
    processor_class is already recorded in processor_config.json.
    """
    try:
        ...
        if os.path.isdir(processor_path):
            local_processor_config = os.path.join(processor_path, PROCESSOR_NAME)
            if os.path.exists(local_processor_config):
                processor_config_file = local_processor_config
        if processor_config_file is None:
            processor_config_file = cached_file(processor_path, PROCESSOR_NAME)
        if processor_config_file is None:
            return None

        config_dict, _ = ProcessorMixin.get_processor_dict(processor_config_file)
...
def get_model_processor(processor_path: str, **kwargs):
    ...
    if modeling_backend == "hf":
        ...
        registered_processor_class = _get_registered_processor_class_name(processor_path)
        if registered_processor_class is not None:
            logger.info_rank0(
                f"[PROCESSOR] Loading {registered_processor_class} directly from registered custom processor."
            )
            kwargs.pop("trust_remote_code", None)
            return MODEL_PROCESSOR_REGISTRY[registered_processor_class]().from_pretrained(processor_path, **kwargs)
        try:  # first load from hf, then replace with veomni
